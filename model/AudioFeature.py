...
class AudioFeature:
    ...
    def __init__(self, audio_file, trimSecs=0):
        ...
        self.duration = 0
        self.audio_data, self.sr = librosa.load(self.audio_file, sr=None,mono=True)
    ...
    def extract_features_one_file(self):
        ...
        if self.trimSecs > 0:
            self.audio_data = trim_start_end_file(audioIn=self.audio_data, sr=self.sr, secs=self.trimSecs)
        ...
        zcr = librosa.feature.zero_crossing_rate(y=self.audio_data)

        # Calculate the spectral centroid of the audio signal
        spec_cent = librosa.feature.spectral_centroid(y=self.audio_data, sr=self.sr)

        # Calculate the spectral bandwidth of the audio signal
        spec_bw = librosa.feature.spectral_bandwidth(y=self.audio_data, sr=self.sr)

        # Calculate the mel-frequency cepstral coefficients (MFCCs) of the audio signal
        mfccs = librosa.feature.mfcc(y=self.audio_data, sr=self.sr,n_mfcc=30)
        ...
        mel_spec = librosa.feature.melspectrogram(y=self.audio_data, sr=self.sr, n_mels=128, fmax=8000)
        ...
        #special contrast
        spec_con = librosa.feature.spectral_contrast(y=self.audio_data, sr=self.sr, n_bands=3)
        spec_con_mean = spec_con.mean(axis=1).T
        spec_con_std = spec_con.std(axis=1).T

        #chroma_stft
        stft = np.abs(librosa.stft(y=self.audio_data))
        ...
        threshold = 30
        intervals = librosa.effects.split(y=self.audio_data, top_db=threshold,
